...
(function($) { // Hide scope, no $ conflict
 ...
 $.datepick.newDate(2014, 12, 25) */
		...
		_postAttach: function(elem, inst) {
			if (inst.inline) {
				...
				elem.bind('keydown.' + inst.name, this._keyDown).bind('keypress.' + inst.name, this._keyPress).
					bind('keyup.' + inst.name, this._keyUp); 
		...
		_attachments: function(elem, inst) {
			...
			if (inst.options.showOnFocus) {
				elem.bind('focus.' + inst.name, this.show);
			}
			if (inst.trigger) {
				inst.trigger.remove();
			}
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
	...
	$(function() {
		$(document).bind('mousedown.' + pluginName, plugin._checkExternalClick).
			bind('resize.' + pluginName, function() { plugin.hide(plugin.curInst); });
	});
