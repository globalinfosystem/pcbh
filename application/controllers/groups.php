...
class Groups extends SB_Controller {
    ...
    function save() {
        $validation_rules = $this->validateForm();
        if ($this->validateFormPassed($validation_rules)) {
            ...
            redirect(site_url('groups', 301));
        } else {
            $this->session->set_flashdata('message', SiteHelpers::alert('error', ""));
            redirect('groups/add/', 301);
        }
    }
