...
class Tblusers extends SB_Controller 
{

	protected $layout 	= "layouts/main";
	public $module 		= 'tblusers';
	public $per_page	= '10';

	function __construct() {
		parent::__construct();
		
		$this->load->model('tblusersmodel');
		$this->model = $this->tblusersmodel;
	...
	function index() 
	{
		...
		$this->data['content'] = $this->load->view('administrator/tblusers/index',$this->data, true );
	...
	function show( $id = null) 
	{
		...
		$this->data['id'] = $id;
		$this->data['content'] =  $this->load->view('administrator/tblusers/view', $this->data ,true);	  
		$this->load->view('layouts/main',$this->data);
	}
	...
	function add( $id = null ) 
	{
		...
		if($row)
		{
			...
			$this->data['row']["password"]=$this->encriptar('decrypt', $this->data['row']["password"]);
		} else {
			$this->data['row'] = $this->model->getColumnTable('tb_users'); 
		}
	    
		$this->data['id'] = $id;
		$this->data['content'] = $this->load->view('administrator/tblusers/form',$this->data, true );		
	  	$this->load->view('layouts/main', $this->data );
	...
	function save() {
		...
		if( $this->form_validation->run() )
		{
			...
			$data["active"]=1;
			$data["password"]=$this->encriptar('encrypt', $data["password"]);
			...
			{
				redirect( 'administrator/tblusers/add/'.$ID,301);
			} else {
				redirect( 'administrator/tblusers',301);
			}			
