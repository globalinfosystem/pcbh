...
class SB_Model extends CI_Model {
	...
	public function getRowCustom($year,$reg,$field) {
        ...
        $query = $this->db->query(
                $this->querySelect() .
                $this->queryWhere() .
                " AND " . $table . "." . $key . " = '{$year}' AND ". $table . "." .$field." = '{$reg}' " .
                $this->queryGroup()
        );
    ...
    function getComboselect($params, $nested = array()) 
	{
		...
		if (isset($params[3]) AND ! empty($params[4])) {
			...
			if($table == 'complaint_mast'){
				$this->db->where('comp_no IN (2,3,5)');
			}
	...
	function getComboselectSubstage($params, $nested = array()) {
        if (isset($params[3]) AND ! empty($params[4])) {
            $table = $params[0];
			$this->db->where('stage_no',$params[3]);
    ...
    function getComboselectCommisoner($params, $nested = array()) {
        if (isset($params[3]) AND ! empty($params[4])) {
            $this->db->where('employee.desig IN (1,2)');
            $this->db->where('tb_users.active IN (1)');
			
            $this->db->join('tb_users','tb_users.empid=employee.empid');
            $query = $this->db->get_where($table, array($param[3] => $param[4]));
            $rows = $query->result();
        } else {
            $this->db->where('employee.desig IN (1,2)');
