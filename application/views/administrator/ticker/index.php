  ...
  <div class="page-content row">
    ...
    <div class="toolbar-line ">		
		...
		<a href="<?php echo site_url('administrator/ticker/add') ?>" class="tips btn btn-xs btn-info"  title="<?php echo $this->lang->line('core.btn_new'); ?>">
		...
		<a href="<?php echo site_url('administrator/ticker/download') ?>" class="tips btn btn-xs btn-default" title="<?php echo $this->lang->line('core.btn_download'); ?>">
		<i class="fa fa-download"></i>&nbsp;<?php echo $this->lang->line('core.btn_download'); ?></a>
		<?php endif;
		if($this->session->userdata('gid') ==1) : ?>	
		<a href="<?php echo site_url('administrator/module/config/ticker') ?>" class="tips btn btn-xs btn-default"  title="<?php echo $this->lang->line('core.btn_config'); ?>">
		<i class="fa fa-cog"></i>&nbsp;<?php echo $this->lang->line('core.btn_config'); ?></a>
		<?php endif; ?>		

	</div>
	<?php echo $this->session->flashdata('message');?>
	 <form action='<?php echo site_url('administrator/ticker/destroy') ?>' class='form-horizontal' id ='SximoTable' method="post" >
        ...
        <tbody>
			...
			<?php foreach ( $rowData as $i => $row ) : ?>
                <tr>
					...
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="<?php echo $row->ticker_id ?>" />  </td>
				 ...
				 <td>
					 <div class="btn-group">		
						...
						<li><a href="<?php echo site_url('administrator/ticker/show/'.$row->ticker_id)?>"  class="tips "  title="view"><i class="fa  fa-search"></i> <?php echo $this->lang->line('core.btn_view'); ?> </a></li>
						<?php endif;
						if($access['is_edit'] ==1) : ?>
						<li><a  href="<?php echo site_url('administrator/ticker/add/'.$row->ticker_id)?>"  class="tips "  title="edit"> <i class="fa fa-edit"></i>  <?php echo $this->lang->line('core.btn_edit'); ?> </a> </li>
						<?php endif;?>
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','<?php echo site_url("administrator/ticker/multisearch");?>');
		$('#SximoTable').submit();
	});
